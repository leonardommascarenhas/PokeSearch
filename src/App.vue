...
export default {
  ...
  methods: {
    ...
    async fetchPokemon(pokemonName) {
        let pokemon = await axios.get(`https://pokeapi.co/api/v2/pokemon/${pokemonName.replace("'", "").replace(/[.: ']+/g, "-").toLowerCase()}`).catch((err) => this.handleError(err));
        ...
        for (let i = 0; i < speciesNames.length; i++) {
          let res = await axios.get(`https://pokeapi.co/api/v2/pokemon/${speciesNames[i]}`);
